...
const getApiUrl = (endpoint) => {
  if (API_BASE) return `${API_BASE}${endpoint}`;
  return endpoint;
};
...
function AiAssistant() {
  ...
  const subscribe = async () => {
    ...
    setLoading(true);
    try {
      await fetch(getApiUrl("/telegram/subscribe"), {
        method: "POST",
        headers: { "Content-Type": "application/json" },
...
export default function App() {
  ...
  const handleFile = async (f) => {
    ...
    try {
      const uploadRes = await fetch(getApiUrl("/upload"), { method: "POST", body: form });
      const { file_id } = await uploadRes.json();

      const startRes = await fetch(getApiUrl("/investigate"), {
        method: "POST",
        headers: { "Content-Type": "application/json" },
  ...
  const startPolling = (id) => {
    pollRef.current = setInterval(async () => {
      try {
        const res  = await fetch(getApiUrl(`/investigation/${id}`));
        const data = await res.json();
